...
public class Target : MonoBehaviour
{
    ...
    public bool hitBoundary = false; // Biến kiểm tra xem đã chạm biên hay chưa
    ...
    void Update()
    {
        if (LevelDesign.Instance.level == 12)
        {
            ...
            {
                RespawnTarget();
                hitBoundary = false; // Đặt lại biến hitBoundary về false sau khi respawn
            }
        }
    }
    ...
    {
        GameplayManager.Instance.GetComponent<LevelDesign>().activeTime = 0f; // Reset thời gian hoạt động của targetPrefab
    ...
    void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("ElvenArrow")) // Nếu bị bắn trúng bởi mũi tên
        {
            ...
            GameplayManager.Instance.TargetHitSound();
            GameplayManager.Instance.RegisterHit(); // Ghi nhận điểm
            GameplayManager.Instance.GetComponent<LevelDesign>().SizeChange();
            LevelDesign.Instance.OnTargetHit();
        }
    }
}